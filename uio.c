...
#define O_RDWR   2
*/
int tokenParse(char *buf, char *entries[], char token)
{
  int entryIndex = 0;
  char *cp;
  ...
  while (*cp != 0) // not null
  {
    while (*cp == ' ')               // skip blank values
      *cp++ = 0;                     // blanks are null and iterate character pointer
    if (*cp != 0)                    // if not null
      entries[entryIndex++] = cp;    //add into entries list
    while (*cp != token && *cp != 0) // scan over word until token chars
      cp++;
    if (*cp != 0) // Null token
      *cp = 0;
    else // End of buffer we can quit
      break;
    cp++;
  }

  return entryIndex; // returns 0 based total entries
}
